...
/*   Updated: 2020/02/04 19:31:54 by alesanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_sort(t_all *all)
{
	...
	while (i < all->spr.nbsp)
	{
		...
		all->sprites[i].dist = ft_fpower(all->cam.pos.x - all->sprites[i].coor.x,
				2) + ft_fpower(all->cam.pos.y - all->sprites[i].coor.y, 2);
		i++;
	}
}
#include <stdio.h>
